...
const Dinner = () => {
  ...
  const addtocart = async (name,price) => {
    ...
    try {
      const res = await axios.post('http://localhost:3000/food/addcart', { name,price,email,state:"Dinner" });
      if (res.status === 200) {
  ...
  return (
   ...
   <header className="dashboard-header">
      ...
      <nav className="dashboard-nav">
        <Link to='/Dashboard'><button className="nav-link active">Tiffin</button>
        </Link>
        <Link to='/Lunch'><button className="nav-link">Lunch</button></Link>
        <Link ><button className="nav-link">Dinner</button></Link>
      </nav>
